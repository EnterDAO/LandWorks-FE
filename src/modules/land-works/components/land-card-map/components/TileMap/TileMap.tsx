...
export class TileMap extends React.PureComponent<Props, State> {
  ...
  handlePanZoom = (args: { dx: number; dy: number; dz: number }) => {
    ...
    const newZoom = Math.max(minZoom, Math.min(maxZoom, zoom - dz * this.getDzZoomModifier()));
  ...
  render() {
    ...
    return (
      <div className={classes} style={styles}>
        <canvas className={this.getCanvasClassName()} width={width} height={height} ref={this.refCanvas} />
      </div>
    );
  }
}
