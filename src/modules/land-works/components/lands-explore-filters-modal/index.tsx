...
export const ExploreFiltersModal: React.FC<Props> = (props) => {
  const MAX_DISTANCE = 9;
  const { onCancel, ...modalProps } = props;
  const [selectedType, setSelectedType] = useState<number>(0);
  const [minSize, setMinSize] = useState<number>(1);
  const [maxSize, setMaxSize] = useState<number>(props.maxLandSize || 10);
            ...
            <Grid container>
              ...
              <Grid xs={4} item>
                ...
                <Input
                  ...
                  onChange={(e) => {
                    const value = +e.target.value;
                    value >= minSize && value <= props.maxLandSize && setMaxSize(value);
                  }}
                  style={{ width: '100%' }}
                  value={maxSize}
                />
